...
public class ProductCategoryBuilder
{
	...
	public void validateProducts()
	{
		...
		{
			if(!product.getFieldNames().contains(ProductConstants.PRODUCT_CATEGORY))
			{
				throw new IllegalStateException("Product must have field " + ProductConstants.PRODUCT_CATEGORY);
			}

			if(!product.getFieldNames().contains(ProductConstants.PRODUCT_QUANTITY))
			{
				throw new IllegalStateException("Product must have field " + ProductConstants.PRODUCT_QUANTITY);
			}

			if(!product.getFieldNames().contains(ProductConstants.PRODUCT_PRICE))
			{
				throw new IllegalStateException("Product must have field " + ProductConstants.PRODUCT_PRICE);
			}

			if(!product.getFieldNames().contains(ProductConstants.PRODUCT_UNIT_PRICE))
			{
				throw new IllegalStateException("Product must have field " + ProductConstants.PRODUCT_UNIT_PRICE);
			}
		}
	}
